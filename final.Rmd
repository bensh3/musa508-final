...
delay <- lapply(paste0('delay/',delay_files),read_csv) %>% map(anti_join,invalid,by=c("date","train_id"))
...
necl.eb.id <- necl2019 %>% filter(stop_sequence == 1 & from == 'Trenton') %>% 
  distinct(train_id,date)
necl.wb.id <- necl2019 %>% filter(stop_sequence == 1 & from == 'New York Penn Station') %>% 
  distinct(train_id,date)
```
  
```{r}
necl.oneday %>% filter(train_id %in% sample(filter(necl.eb.id, date == ymd('20190618')),10)) %>%
