...
We begin model training by log-transforming the delay time, in order to compensate for the natural left skew. Then create a training and testing dataset by splitting the dataset to before and after August.
...
When we plot the predicted average delay against the observed delay, we notice that while "regular" phugoidal small delays of 4 minutes of less are well-predicted against, large delays of 7 minutes or more are more intermittently anticipated and predicted.
...
Incorporating cross-validation across space and time will help us understand where and how our predictions are holding up, as a measure of generalizability. 
...
On a macro level, our model appears to work with relative accuracy of `r mape` percent. This would indicate a fairly successful proof-of concept on paper for including delay predictions as a feature on customer-facing NJ Transit applications. 
