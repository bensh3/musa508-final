...
```{r intake GTFS data}
njt <- read_gtfs("gtfs_njt.zip") %>% filter_feed_by_stops(stop_ids=c('148'))
...
necl <- rbindlist(delay %>% tidyselect:::select(starts_with('2018') | starts_with('2019'))) %>% 
...
necl.id <- necl %>% distinct(train_id,date,direction)
```

```{r}
first <- necl %>% group_by(date,train_id) %>% arrange(stop_sequence) %>% filter(row_number()==1) %>% rename('first_time' = 'actual_time') %>% select(date,train_id,first_time)

last <- necl %>% group_by(date,train_id) %>% arrange(-stop_sequence) %>% filter(row_number()==1) %>% rename('last_time' = 'actual_time') %>% select(date,train_id,last_time)

invalid2 <- first %>% left_join(last,by=c('date','train_id')) %>% filter(first_time == last_time)

necl <- necl %>% anti_join(invalid2,by=c('date','train_id'))
```
...
p1 <- necl %>% filter(isWkdy) %>% group_by(direction,period,stop_seq_abs) %>%
  ...
  ggtitle('Weekday')
p2 <- necl %>% filter(!isWkdy) %>% group_by(direction,period,stop_seq_abs) %>%
  summarise(avgdelay = mean(delay_minutes)) %>% 
  ggplot(aes(x=stop_seq_abs, y=avgdelay)) +
  geom_line() +
  facet_grid(direction ~ factor(period,levels=c("AM peak","Midday","PM peak","Evening","Overnight"))) +
...
```{r stringline diagram, fig.width=8, fig.height=6}
oneday <- necl %>% filter(date %in% slice_sample(.)) %>%
...
ggplot(cancel, aes(fill=reorder(CATEGORY,-CANCEL_COUNT),
                   x=factor(str_sub(str_to_sentence(cancel$MONTH),1,3),level=month.abb),
                   y=CANCEL_COUNT,)) +
  geom_bar(position="stack",stat="identity") +
  scale_fill_discrete(name='Category') +
  labs(title='NJT cancellations by month and reason', subtitle="Line:",
...
necl <- necl %>% mutate(YEAR=as.character(year(date)),MONTH=str_to_upper(month(date, label=T, abbr=F))) %>% 
  left_join(cancel,by=c('YEAR','MONTH')) %>% mutate_if(is.numeric,coalesce,0) %>% 
...
```{r}
necl <- necl %>% left_join(dplyr::select(njt_sf$stops,stop_id,zone_id,geometry),by=c("to_id"="stop_id")) %>% st_as_sf()
```
...
necl <- necl %>% mutate(logDelay = log(delay_minutes + 0.1))

delay.Train <- filter(necl, date < ymd(20190701))
delay.Test <- filter(necl, date >= ymd(20190701))
```

## Regression formula

```{r}
fit <- lm(logDelay ~ to + period + isWkdy + direction + Precipitation +
...
reg.cv.station %>%
  ...
  ggplot() + 
    geom_histogram(aes(MAE), bins = 30) +
    geom_vline(xintercept = 0) + 
    labs(title="Cross-validation: mean average error", subtitle = "k-fold cross validation: station",
         x="Mean Absolute Error", y="Count")
```
